...
def create_train_env(level, output_path=None): 
    env = retro.make("Contra-Nes", state="Level{}".format(level), use_restricted_actions=retro.Actions.FILTERED, inttype=retro.data.Integrations.EXPERIMENTAL_ONLY)
    if output_path:
        monitor = Monitor(224, 240, output_path)
    else:
        monitor = None
